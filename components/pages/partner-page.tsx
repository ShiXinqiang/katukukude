...
export function PartnerPage({onBack}:{onBack:()=>void}) {
 ...
 const submit=()=>{
   if(!form.name.trim()||form.advantages.trim().length<10||!Object.values(form.channels).some(Boolean)) return;
   const drafts=JSON.parse(localStorage.getItem("katu.partner-applications")||"[]");
   localStorage.setItem("katu.partner-applications",JSON.stringify([...drafts,{...form,id:crypto.randomUUID(),createdAt:new Date().toISOString()}]));
   setSent(true);
 };
 ...
 <button type="button" onClick={submit} className="flex w-full items-center justify-center gap-2 rounded-2xl bg-[#7189a1] py-4 text-sm font-bold text-white"><Send size={18}/>提交合作意向</button>
 </div></main>;
}
