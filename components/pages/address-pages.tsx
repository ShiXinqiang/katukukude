...
export function AddressesPage({onBack}:{onBack:()=>void}) {
 ...
 const [locating,setLocating]=useState(false); const [hint,setHint]=useState("");
 ...
 if(editing) return <main className="min-h-screen bg-[#f5f7f9]"><Header title="添加地址" onBack={()=>setEditing(false)}/><div className="space-y-3 p-4">
   ...
   <section className="rounded-3xl bg-white p-4 shadow-sm"><div className="flex items-center gap-2"><MapPin size={18} className="text-[#7189a1]"/><b className="text-sm text-slate-800">Google 地图位置链接</b></div><input value={form.googleMapsUrl} onChange={e=>setForm({...form,googleMapsUrl:e.target.value})} placeholder="粘贴 Google Maps 分享链接" className="mt-3 w-full rounded-2xl bg-slate-50 px-3 py-3 text-sm outline-none"/><p className="mt-2 text-[11px] leading-5 text-slate-400">打开地图，长按准确位置后点“分享”，复制链接并粘贴。只有带地图链接的地址才能用于“到家”导航。</p><button type="button" onClick={()=>{if(!navigator.geolocation){setHint("当前浏览器不支持定位");return;}setLocating(true);navigator.geolocation.getCurrentPosition(({coords})=>{setForm({...form,googleMapsUrl:"https://www.google.com/maps/search/?api=1&query="+coords.latitude+","+coords.longitude});setHint("已填入当前位置，请确认后保存");setLocating(false)},()=>{setHint("定位失败，请检查浏览器权限");setLocating(false)},{enableHighAccuracy:true,timeout:12000})}} className="mt-3 flex w-full items-center justify-center gap-2 rounded-2xl border border-[#9dafbf] py-3 text-sm font-semibold text-[#607d96]"><Navigation size={17}/>{locating?"正在获取当前位置…":"自动填写当前位置"}</button><a href="https://www.google.com/maps" target="_blank" rel="noreferrer" className="mt-2 flex w-full items-center justify-center gap-2 py-2 text-xs text-slate-500">打开 Google 地图手动复制链接</a>{hint&&<p className="mt-2 text-center text-xs text-[#607d96]">{hint}</p>}</section>
