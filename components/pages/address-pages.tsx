...
export function AddressesPage({onBack}:{onBack:()=>void}) {
 ...
 const autoFill = () => {
   if(!navigator.geolocation){setHint("当前浏览器不支持定位");return;}
   setLocating(true);
   navigator.geolocation.getCurrentPosition(async ({coords})=>{
     ...
     try{
       ...
       setForm(prev=>({...prev,googleMapsUrl:mapUrl,detail:result?.detail||prev.detail}));
       setHint(result?.label?"已识别："+result.label+"，请确认后补充门牌":"已填入当前位置，请确认后保存");
     }catch{setForm(prev=>({...prev,googleMapsUrl:mapUrl}));setHint("已填入当前位置，请确认后保存");}
     finally{setLocating(false);}
   },()=>{setHint("定位失败，请检查浏览器权限");setLocating(false)},{enableHighAccuracy:true,timeout:12000});
 };
