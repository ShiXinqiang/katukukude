...
export function AddressesPage({onBack}:{onBack:()=>void}) {
 ...
 const submit=()=>{
   ...
   const next=[...items,{...form,id:crypto.randomUUID(),isHome:items.length===0}];persist(next);setEditing(false);setForm({label:"家",contact:"",phone:"",detail:"",googleMapsUrl:"",latitude:"",longitude:""});
 };
 ...
 if(editing) return <main className="min-h-screen bg-[#f5f7f9]"><Header title="添加地址" onBack={()=>setEditing(false)}/><div className="space-y-3 p-4">
   <section className="overflow-hidden rounded-3xl bg-white px-4 shadow-sm">
    {[["地址标签","label","例如：家、公司"],["联系人","contact","收货人姓名"],["手机号","phone","中国或缅甸手机号"],["详细地址","detail","省/邦、城市、镇区、村寨、社区、街道门牌"]].map(([label,key,placeholder])=><label key={key} className={`flex border-b border-slate-100 py-4 last:border-0 ${key==="detail"?"items-start":"items-center"}`}><span className="w-20 shrink-0 text-sm font-medium text-slate-700">{label}</span>{key==="detail"?<textarea rows={3} value={form.detail} onChange={e=>setForm({...form,detail:e.target.value})} placeholder={placeholder} className="min-w-0 flex-1 resize-none text-sm leading-6 outline-none placeholder:text-slate-300"/>:<input value={form[key as keyof typeof form]} onChange={e=>setForm({...form,[key]:e.target.value})} placeholder={placeholder} className="min-w-0 flex-1 text-sm outline-none placeholder:text-slate-300"/>}</label>)}
   </section>
   <section className="rounded-3xl bg-white p-4 shadow-sm"><div className="flex items-center gap-2"><MapPin size={18} className="text-[#7189a1]"/><b className="text-sm text-slate-800">Google 地图位置链接</b></div><input value={form.googleMapsUrl} onChange={e=>setForm({...form,googleMapsUrl:e.target.value})} placeholder="粘贴 Google Maps 分享链接" className="mt-3 w-full rounded-2xl bg-slate-50 px-3 py-3 text-sm outline-none"/><p className="mt-2 text-[11px] leading-5 text-slate-400">打开地图，长按准确位置后点“分享”，复制链接并粘贴。只有带地图链接的地址才能用于“到家”导航。</p><button type="button" onClick={autoFill} className="mt-3 flex w-full items-center justify-center gap-2 rounded-2xl border border-[#9dafbf] py-3 text-sm font-semibold text-[#607d96]"><Navigation size={17}/>自动定位或手动选择地点</button><p className="mt-2 text-center text-[11px] leading-5 text-slate-400">如需手动选点，请点击上面的按钮；地图会在新页面打开，复制链接后返回本页面粘贴。</p>{hint&&<p className="mt-2 text-center text-xs text-[#607d96]">{hint}</p>}</section>
   <button type="button" onClick={submit} className="w-full rounded-2xl bg-[#7189a1] py-4 text-sm font-bold text-white">保存地址</button>
 </div>{locationPickerOpen&&<LocationPicker title="填写收货地址" description="自动定位失败时，可从 Google 地图复制位置链接回来填写。" initialLink={form.googleMapsUrl} onClose={()=>setLocationPickerOpen(false)} onApply={applyLocation}/>}</main>;
