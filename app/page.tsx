...
export default function Page(){
 ...
 useEffect(()=>{try{const mode=localStorage.getItem("katu.theme.mode")||"balanced",savedAccent=localStorage.getItem("katu.theme.accent"),accent=!savedAccent||savedAccent==="106 132 157"?"176 126 89":savedAccent,theme=mode==="clear"?[".58","26px"]:mode==="solid"?[".86","16px"]:[".72","22px"];document.documentElement.style.setProperty("--katu-glass-alpha",theme[0]);document.documentElement.style.setProperty("--katu-blur",theme[1]);document.documentElement.style.setProperty("--katu-accent",accent);document.documentElement.style.setProperty("--katu-accent-strong",accent==="194 132 67"?"163 96 39":accent);if(!savedAccent||savedAccent==="106 132 157")localStorage.setItem("katu.theme.accent",accent)}catch{}},[]);
