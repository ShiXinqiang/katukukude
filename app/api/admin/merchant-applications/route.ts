...
export async function GET(request: Request) {
  try {
    ...
    if (!allowed.includes(status)) return NextResponse.json({ message: "无效的审核状态" }, { status: 400 });
    ...
    const result = await database.query<MerchantApplicationRecord>(
      `SELECT a.id, a.user_id AS "userId", u.username, u.display_name AS "displayName",
              a.store_name_cn AS "storeNameCn", a.store_name_mm AS "storeNameMm",
              a.legal_name AS "legalName", a.contact_name AS "contactName",
              a.phone, a.email, a.business_type AS "businessType",
              a.license_no AS "licenseNo", a.identity_no AS "identityNo",
              a.state_region AS "stateRegion", a.city, a.township, a.address,
              a.map_link AS "mapLink", a.description,
